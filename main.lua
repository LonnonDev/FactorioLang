package.path = package.path .. ';local/share/lua/5.4/?.lua'

local base64 = require("base64")
local zlib = require('zlib')
local io = require("io")

local generatedjson = "{\"blueprint\": {\"icons\": [{\"signal\": {\"type\": \"item\",\"name\": \"decider-combinator\"},\"index\": 1}],\"entities\": ["
local position = {0, 0}
local circuits = {}

function decode()
    local blueprint = ""
    local inflated = zlib.decompress(base64.decode(blueprint))
    file = io.open("decoded.json", "w")
    io.output(file)
    io.write(inflated)
    io.close()
    io.output()
end

function encode()
    generatedjson = generatedjson .. "],\"item\": \"blueprint\", \"version\": 281479274430464}}"
    local encoded = zlib.compress(generatedjson, 9)
    local base64encoded = base64.encode(encoded);
    print(generatedjson)
    print("0" .. base64encoded)
end

function generate()
    encode()
end

function decider(first, comparator, second, connections)
    local circuitslen = 0
    for _ in pairs(circuits) do circuitslen = circuitslen + 1 end

    local root = ""
    local compare = ""
    if type(first) == "number" then 
        root = first
    elseif type(first) == "string" then
        root = "{\"type\": \"virtual\",\"name\": \"signal-" .. first .. "\"},"
    end
    if type(second) == "number" then 
        compare = second
    elseif type(second) == "string" then
        compare = "{\"type\": \"virtual\",\"name\": \"signal-" .. second .. "\"},"
    end
    if circuitslen == 0 then
        generatedjson = generatedjson .. "{\"entity_number\":" .. circuitslen + 1 .. ",\"name\": \"decider-combinator\",\"position\": { \"x\": " .. position[1] .. ", \"y\": " .. position[2] .. "},\"control_behavior\": {\"decider_conditions\": {\"first_signal\": " .. root .. " \"second_signal\": " .. compare .. "\"comparator\": \"" .. comparator .. "\", \"copy_count_from_input\": true}}}" 
    else
        generatedjson = generatedjson .. "," .. "{\"entity_number\":" .. circuitslen + 1 .. ",\"name\": \"decider-combinator\",\"position\": { \"x\": " .. position[1] .. ", \"y\": " .. position[2] .. "},\"control_behavior\": {\"decider_conditions\": {\"first_signal\": " .. root .. "\"second_signal\": " .. compare .. "\"comparator\": \"" .. comparator .. "\", \"copy_count_from_input\": true}}}" 
    end
    return {1}
end