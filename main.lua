...
function decider(statement, output, connections)
    ...
    local outputsignal = ""
    -- Chekcs if any of the inputs are constants or strings and them puts them accordingly
    ...
    if circuits == 0 then
        generatedjson = generatedjson .. "{\"entity_number\":" .. circuits + 1 .. ",\"name\": \"decider-combinator\",\"position\": { \"x\": " .. position[1] .. ", \"y\": " .. position[2] .. "},\"control_behavior\": {\"decider_conditions\": {\"first_signal\": " .. first .. "\"second_signal\": " .. second .. "\"comparator\": \"" .. comparator .. "\",\"output_signal\":" .. outputsignal .. "\"copy_count_from_input\": true}}}" 
    else
        generatedjson = generatedjson .. "," .. "{\"entity_number\":" .. circuits + 1 .. ",\"name\": \"decider-combinator\",\"position\": { \"x\": " .. position[1] .. ", \"y\": " .. position[2] .. "},\"control_behavior\": {\"decider_conditions\": {\"first_signal\": " .. first .. "\"second_signal\": " .. second .. "\"comparator\": \"" .. comparator .. "\",\"output_signal\":" .. outputsignal .. "\"copy_count_from_input\": true}}}" 
    end
    circuits = circuits + 1
